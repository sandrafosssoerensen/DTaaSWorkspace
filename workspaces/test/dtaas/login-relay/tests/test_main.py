...
class TestIsRoutable:
    ...
    def test_workspace_redirect_routable(self):
        """workspace-redirect paths are routable so login return_to is preserved."""
        assert _is_routable("/workspace-redirect/functions/myfile.py") is True
        assert _is_routable("/workspace-redirect/common/data") is True
        assert _is_routable("/workspace-redirect/tree/functions") is True
        assert _is_routable("/workspace-redirect/tree/common") is True

    def test_workspace_redirecttree_routable(self):
        """workspace-redirecttree paths are routable (SPA strips trailing slash from LIBLINK)."""
...
class TestWorkspaceRedirectGeneric:
    ...
    Used for library file links (REACT_APP_URL_LIBLINK='workspace-redirect').
    The SPA constructs these as {REACT_APP_URL}workspace-redirect/{path};
    login-relay resolves the username from the cookie and redirects to /{username}/{path}.
    """

    def test_authenticated_user_redirected_to_path(self):
        """Valid cookie redirects to /{username}/{path}."""
        future = int(time.time()) + 3600
        token = _make_jwt({"preferred_username": "user1", "exp": future})
        c = TestClient(app, follow_redirects=False, cookies={"dtaas_access_token": token})
        resp = c.get("/workspace-redirect/functions/myfile.py")
        assert resp.status_code == 302
        assert resp.headers["location"] == "/user1/functions/myfile.py"

    def test_nested_path_redirected_correctly(self):
        """Nested library path is resolved per authenticated user."""
        future = int(time.time()) + 3600
        token = _make_jwt({"preferred_username": "user2", "exp": future})
        c = TestClient(app, follow_redirects=False, cookies={"dtaas_access_token": token})
        resp = c.get("/workspace-redirect/common/data")
        assert resp.status_code == 302
        assert resp.headers["location"] == "/user2/common/data"

    def test_query_string_preserved(self):
        """Query string parameters are forwarded to the resolved workspace path."""
        future = int(time.time()) + 3600
        token = _make_jwt({"preferred_username": "user1", "exp": future})
        c = TestClient(app, follow_redirects=False, cookies={"dtaas_access_token": token})
        resp = c.get("/workspace-redirect/functions/script.py?foo=bar")
        assert resp.status_code == 302
        assert resp.headers["location"] == "/user1/functions/script.py?foo=bar"
    ...
    def test_missing_cookie_redirects_to_login(self):
        ...
        resp = client.get("/workspace-redirect/lab")
        assert resp.status_code == 302
        assert resp.headers["location"] == "/login-relay?return_to=/workspace-redirect/lab"
...
class TestWorkspaceRedirectTree:
    """Tests for the /workspace-redirecttree/{path} folder-browser redirect.

    The SPA strips the trailing slash from REACT_APP_URL_LIBLINK before appending
    'tree/{dir}', producing /workspace-redirecttree/{dir} instead of
    /workspace-redirect/tree/{dir}. Login-relay redirects to /{username}/tree/{dir}.
    """

    def test_authenticated_user_redirected_to_tree_path(self):
    ...
    def test_nested_path_redirected_correctly(self):
        """Nested folder path is resolved per authenticated user."""
        future = int(time.time()) + 3600
        token = _make_jwt({"preferred_username": "user2", "exp": future})
        c = TestClient(app, follow_redirects=False, cookies={"dtaas_access_token": token})
